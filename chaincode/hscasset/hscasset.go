...
export CHAINCODE_NAME=hscasset
export CHAINCODE_SRC=github.com/chaincode/hscasset
...
peer chaincode  list --installed -C marketplace
...
peer chaincode instantiate -o orderer.midominio.com:7050 --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CHAINCODE_NAME -v $CHAINCODE_VERSION -c '{"Args":[]}' -P "OR ('Org1MSP.member','Org2MSP.member','Org3MSP.member')"
peer chaincode  list --instantiated -C marketplace
peer chaincode invoke -o orderer.midominio.com:7050  --tls --cafile $ORDERER_CA  -C $CHANNEL_NAME -n $CHAINCODE_NAME -c '{"Args":["init","100","Xray-Machine","0e83ff"]}'
peer chaincode invoke -o orderer.midominio.com:7050  --tls --cafile $ORDERER_CA  -C $CHANNEL_NAME -n $CHAINCODE_NAME -c '{"Args":["Order", "100","initial order from Administration Office", "New York"]}'
peer chaincode query -o orderer.midominio.com:7050  --tls --cafile $ORDERER_CA  -C $CHANNEL_NAME -n $CHAINCODE_NAME -c '{"Args":["query","100"]}'
peer chaincode invoke -o orderer.midominio.com:7050  --tls --cafile $ORDERER_CA  -C $CHANNEL_NAME -n $CHAINCODE_NAME -c '{"Args":["Ship", "100", "OEM deliver Xray-Machine to Administration Office", "Dubai"]}'
peer chaincode invoke  -o orderer.midominio.com:7050  --tls --cafile $ORDERER_CA  -C $CHANNEL_NAME -n $CHAINCODE_NAME -c '{"Args":["Issue", "100","Issued Xray-Machine to hospital1", "Abu Dhabi"]}'
peer chaincode query -o orderer.midominio.com:7050  --tls --cafile $ORDERER_CA  -C $CHANNEL_NAME -n $CHAINCODE_NAME -c '{"Args":["getHistory","100"]}'
*/
